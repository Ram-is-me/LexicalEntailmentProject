...
class ExperimentRun:
    ...
    def __init__(self, config_filepath):
        ...
        # NEW LINE HERE TODO
        dictionary_list = self.config.get("data","conv").replace("[","").replace("]", "").replace(" ", " ").split(",") # TODO DONE
        ...
        if dictionary_list != "":
            for dictionary_filepath in dictionary_list:
                if dictionary_filepath != "":
                    self.dictionary = self.dictionary | self.load_constraints(dictionary_filepath)
    ...
    def initialise_model(self, numpy_embedding):
        ...
        # TODO NEW LINE HERE
        self.dictionary_constant = tf.placeholder("float")
        ...
        # TERM FOR SYNONYMS ACROSS LANGUAGES - L2 Distance from ATTRACT CODE TODO
        dictionary_cost_attract_mono = self.lambda_d * (tf.nn.l2_loss(original_attract_examples_left_mono - attract_examples_left_mono) + tf.nn.l2_loss(original_attract_examples_right_mono - attract_examples_right_mono))
        self.attract_cost_mono += dictionary_cost_attract_mono
        self.attract_cost_mono = self.attract_cost_mono * 1.0

        dictionary_cost_attract_cl = self.lambda_d * (tf.nn.l2_loss(original_attract_examples_left_cl - attract_examples_left_cl) + tf.nn.l2_loss(original_attract_examples_right_cl - attract_examples_right_cl))
        self.attract_cost_cl += dictionary_cost_attract_cl
        self.attract_cost_cl = self.attract_cost_cl * 1.0
        
        # dictionary_cost_repel = self.lambda_d * (tf.nn.l2_loss(original_repel_examples_left - repel_examples_left) + tf.nn.l2_loss(original_repel_examples_right - repel_examples_right))
        # self.repel_cost += dictionary_cost_repel
        # self.repel_cost += self.repel_cost * 1.0
    ...
    def load_experiment_hyperparameters(self):
        ...
        # TODO NEW LINE HERE constant lambda d for cost function here
        self.lambda_d = self.config.getfloat("hyperparameters","lambda_d") # TODO DONE
...
def simlex_analysis(word_vectors, language="english", source="simlex", add_prefixes=True):
    ...
    pair_list = []
    if source == "simlex":
        fread_simlex=codecs.open("/content/drive/My Drive/NLP Project/LEAR-master/evaluation/simlex-" + language + ".txt", 'r', 'utf-8')
    elif source == "simlex-old":
        fread_simlex=codecs.open("/content/drive/My Drive/NLP Project/LEAR-master/evaluation/simlex-english-old.txt", 'r', 'utf-8')
    elif source == "simverb":
        fread_simlex=codecs.open("/content/drive/My Drive/NLP Project/LEAR-master/evaluation/simverb.txt", 'r', 'utf-8')
    elif source == "wordsim":
        fread_simlex=codecs.open("/content/drive/My Drive/NLP Project/LEAR-master/evaluation/ws-353/wordsim353-" + language + ".txt", 'r', 'utf-8') # specify english, english-rel, etc.
...
def custom_results(word_vectors, language="english", distance_metric="metric-1", order=2, add_prefixes=False):
    pair_list = []
    fread_simlex = codecs.open("/content/drive/My Drive/NLP Project/LEAR-master/evaluation/my_test_small.txt",'r','utf-8')
    ...
    for (x,y) in pair_list:
        ...
        res.append([word_vectors[word_i], word_vectors[word_j], current_distance])
        #current_distance = distance(word_vectors[word_i], word_vectors[word_j])
        extracted_scores[(word_i, word_j)] = current_distance
        extracted_list.append(((word_i, word_j), current_distance))
    pickle.dump(res, open("vec_new.pickle", "wb"))

    return extracted_list

def custom_analysis(word_vectors, language="english", source="hyperlex", distance_metric="metric-1", order=2, add_prefixes=False):
    pair_list = []
    # if source == "hyperlex":
    #     fread_simlex=codecs.open("/content/drive/My Drive/NLP Project/LEAR-master/evaluation/hyperlex.txt", 'r', 'utf-8')
    # elif source == "hyperlex-nouns":
    #     fread_simlex=codecs.open("/content/drive/My Drive/NLP Project/LEAR-master/evaluation/hyperlex-nouns.txt", 'r', 'utf-8')
    # elif source == "hyperlex-test":
    #     fread_simlex=codecs.open("/content/drive/My Drive/NLP Project/LEAR-master/evaluation/hyperlex_test.txt", 'r', 'utf-8')
    # else:
    #     "Error with HyperLex!"

    fread_simlex = codecs.open("/content/drive/My Drive/NLP Project/LEAR-master/evaluation/my_test.txt",'r','utf-8')
...
def simlex_scores(word_vectors, distance_metric, order, print_simlex=True):
    ...
    for language in ["english", "german", "italian", "russian", "croatian", "hebrew"]:
        ...
        if language == "english":
            ...
            my_test_score, my_test_coverage = custom_analysis(word_vectors, language, source=" ", distance_metric=distance_metric, order = order)
        ...
        if simlex_coverage > 0:
            ...
            if print_simlex:
                ...
                if language == "english":
                    ...
                    print "Custom Test score for ", language, "and cross-lingual is ", my_test_score, "coverage:", my_test_coverage, "/100"
...
def run_experiment(config_filepath):
    ...
    l = custom_results(current_experiment.word_vectors, current_experiment.distance_metric, current_experiment.order), "\n"
    print("Testing outputs: ")
    print(l)

def run_evaluation(config_filepath):
